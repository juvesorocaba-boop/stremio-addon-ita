...
def catalog(tipo, cat_id, extra=None, config_str=None):
    ...
    if tipo == "movie":
        for item in DB.get("filmes", []):
            ...
            if search_query and search_query not in tmdb["titulo"].lower(): continue
            if genre_filter and genre_filter not in tmdb["generi"]: continue
            metas.append({"id": item["id"], "type": "movie", "name": tmdb["titulo"], "poster": tmdb["poster"], "description": tmdb["sinopse"], "genres": tmdb["generi"]})
    elif tipo == "series":
        for item in DB.get("series", []):
            ...
            if search_query and search_query not in tmdb["titulo"].lower(): continue
            if genre_filter and genre_filter not in tmdb["generi"]: continue
            metas.append({"id": item["id"], "type": "series", "name": tmdb["titulo"], "poster": tmdb["poster"], "description": tmdb["sinopse"], "genres": tmdb["generi"]})
    elif tipo == "tv":
        for item in DB.get("canais", []):
            if search_query and search_query not in item["titulo"].lower(): continue
            if genre_filter and item.get("gruppo", "Altro") != genre_filter: continue
            metas.append({"id": item["id"], "type": "tv", "name": item["titulo"], "poster": item.get("poster", ""), "genres": [item.get("gruppo", "Altro")]})
...
def meta(tipo, meta_id, config_str=None):
    if tipo == "movie":
        for item in DB.get("filmes", []):
            if item["id"] == meta_id:
                tmdb = get_tmdb_meta(item["id"], "movie")
                if not tmdb: return jsonify({"meta": {}}), 404
                return jsonify({"meta": {"id": item["id"], "type": "movie", "name": tmdb["titulo"], "poster": tmdb["poster"], "description": tmdb["sinopse"], "genres": tmdb["generi"]}})
    elif tipo == "series":
        for item in DB.get("series", []):
            if item["id"] == meta_id:
                tmdb = get_tmdb_meta(item["id"], "series")
                ...
                for temporada, episodios in item.get("temporadas", {}).items():
                    for episodio in episodios:
                        videos.append({"id": f"{item['id']}:{temporada}:{episodio}", "title": f"S{int(temporada):02d}E{int(episodio):02d}", "season": int(temporada), "episode": int(episodio)})
                return jsonify({"meta": {"id": item["id"], "type": "series", "name": tmdb["titulo"], "poster": tmdb["poster"], "description": tmdb["sinopse"], "genres": tmdb["generi"], "videos": videos}})
    elif tipo == "tv":
        for item in DB.get("canais", []):
            if item["id"] == meta_id:
                return jsonify({"meta": {"id": item["id"], "type": "tv", "name": item["titulo"], "genres": [item.get("gruppo", "Altro")]}})
...
def get_file_size(url):
    try:
        ...
        if length: return format_size(int(length))
...
def build_stream(nome_conteudo, url, extra_linha=None):
    ...
    if len(formato) > 4: formato = None
    linhas = [f"{nome_conteudo} 🇮🇹"]
    detalhe = []
    if formato: detalhe.append(f".{formato}")
    if tamanho: detalhe.append(f"💾 {tamanho}")
    if extra_linha: detalhe.append(extra_linha)
    if detalhe: linhas.append(" | ".join(detalhe))
    return {"name": "Cinema Italiano", "title": "\n".join(linhas), "url": url}
...
def stream(tipo, stream_id, config_str=None):
    ...
    if tipo == "movie":
        ...
        if tmdb_extra:
            ...
            for link in buscar_stream_torrent(nome_filme):
                streams.append(build_stream(nome_filme + " (Extra)", link))
        for item in DB.get("filmes", []):
            if item["id"] == stream_id:
                streams.append(build_stream(item["titulo"] + " (DB)", item["url"]))

    elif tipo == "series":
        parts = stream_id.split(":")
        base_id = parts[0]
        tmdb_extra = get_tmdb_meta(base_id, "series")
        if tmdb_extra:
            nome_serie = tmdb_extra["titulo"]
            for link in buscar_stream_torrent(nome_serie):
                streams.append(build_stream(nome_serie + " (Extra)", link))
        for item in DB.get("series", []):
            if item["id"] == base_id and len(parts) == 3:
                temporada, episodio = parts[1], parts[2]
                url = item.get("temporadas", {}).get(temporada, {}).get(episodio)
                if url:
                    streams.append(build_stream(f"{item['titulo']} S{int(temporada):02d}E{int(episodio):02d} (DB)", url))
